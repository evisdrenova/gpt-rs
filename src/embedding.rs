...
impl Embedding {
    ...
    pub fn forward(&self, token_ids: &Tensor) -> Tensor {
        //   If `token_ids` has shape [batch_size, max_length], then
        //   `weights.index_select(0, token_ids.flatten())` will yield a
        //   [batch_size * max_length, output_dim] matrix. We then reshape to
        //   [batch_size, seq_len, output_dim].

        let (batch_size, max_length) = {
        ...
        // reshape to [batch_size, max_length, output)dim]
        let output_dim = self.weights.dims()[1];
        let out = gathered.view([batch_size, max_length, output_dim]);
